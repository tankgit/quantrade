...
class QuantCLI:
    ...
    async def cmd_account_balance(self, args) -> None:
        ...
        balance = await self.account_manager.get_account_balance(
    ...
    async def cmd_account_positions(self, args) -> None:
        ...
        if positions:
            ...
            print(
                f"{'股票代码':<12} {'股票名称':<12} {'持仓':<8} {'成本价':<10} {'市价':<10} {'市值':<12} {'盈亏':<12} {'盈亏%':<8}"
            )
            print("-" * 100)
            ...
            for pos in positions:
                ...
                print(
                    f"{pos['symbol']:<12} {pos['symbol_name']:<12} {pos['quantity']:<8} "
    ...
    async def cmd_account_orders(self, args) -> None:
        ...
        if orders:
            print(
                f"{'订单号':<12} {'股票代码':<12} {'方向':<6} {'数量':<8} {'价格':<10} {'状态':<10} {'提交时间':<20}"
            )
            print("-" * 100)
            ...
            for order in orders:
                ...
                print(
                    f"{order['order_id']:<12} {order['symbol']:<12} {order['side']:<6} "
    ...
    async def cmd_portfolio_summary(self, args) -> None:
        ...
        if summary:
            ...
            print(
                f"总盈亏: {summary['total_unrealized_pnl']:,.2f} ({summary['total_unrealized_pnl_percent']:.2f}%)"
            )
    ...
    async def cmd_strategy_list(self, args) -> None:
        ...
        if strategies:
            ...
            print(
                f"{'策略名称':<15} {'状态':<8} {'总信号':<8} {'买入信号':<8} {'卖出信号':<8}"
            )
            print("-" * 60)
            ...
            for name, info in strategies.items():
                ...
                print(
                    f"{name:<15} {status:<8} {info['total_signals']:<8} "
    ...
    async def cmd_task_list(self, args) -> None:
        ...
        if tasks:
            print(f"\n=== 任务列表 ({len(tasks)}个) ===")
            print(
                f"{'任务ID':<12} {'任务名称':<15} {'类型':<10} {'状态':<8} {'运行次数':<8} {'错误次数':<8}"
            )
            print("-" * 80)

            for task_id, task in tasks.items():
                print(
                    f"{task_id:<12} {task.name:<15} {task.task_type.value:<10} "
...
def create_parser():
    ...
    # 投资组合摘要
    portfolio_parser = account_subparsers.add_parser(
        "portfolio", help="查询投资组合摘要"
    )
    ...
    strategy_create_parser.add_argument(
        "--short-period", type=int, default=5, help="短期均线周期"
    )
    strategy_create_parser.add_argument(
        "--long-period", type=int, default=20, help="长期均线周期"
    )
    strategy_create_parser.add_argument("--symbols", help="股票代码列表，逗号分隔")
    strategy_create_parser.add_argument(
        "--interval", type=float, default=10.0, help="执行间隔（秒）"
    )
    strategy_create_parser.add_argument("--max-runs", type=int, help="最大执行次数")
    strategy_create_parser.add_argument(
        "--auto-restart", action="store_true", help="自动重启"
    )
