...
class SimpleMAStrategy(BaseStrategy):
    ...
    def __init__(self, short_period: int = 5, long_period: int = 20, symbols: Optional[List[str]] = None):
        params = {
    ...
    def calculate_signals(self, market_data: Dict[str, Any]) -> List[TradingSignal]:
        ...
        # 计算前一期的移动平均（用于判断交叉）
        prev_short_ma = self._calculate_ma(prices[:-1], self.short_period) if len(prices) > self.short_period else None
        prev_long_ma = self._calculate_ma(prices[:-1], self.long_period) if len(prices) > self.long_period else None
